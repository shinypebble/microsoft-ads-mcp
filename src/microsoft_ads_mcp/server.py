...
_INSTRUCTIONS = (
    ...
    "`delete_*`. `update_campaign` / `create_campaign` also set the campaign's inline bid strategy "
    "via `bid_strategy_type` (EnhancedCpc / MaxClicks / MaxConversions / TargetCpa / TargetRoas / "
    "MaxConversionValue / ManualCpc; e.g. MaxClicks + `max_cpc` is Maximize Clicks with a CPC "
    "limit). `set_account_url_options` sets the tracking template / suffix / msclkid "
    "auto-tagging once for the whole account (cleaner than editing every campaign). Also "
    "available: negative keywords "
    "(`add_negative_keywords` / `get_negative_keywords` / `remove_negative_keywords`), "
    "campaign website exclusions -- block referrer domains / mobile-app ids where ads serve "
    "(`get_website_exclusions` / `add_website_exclusions` / `remove_website_exclusions`; "
    "add/remove are additive read-modify-write over Microsoft's replace-all negative-sites "
    "list), ad "
    ...
    "`create_conversion_goal` adds a goal (OfflineConversion or a UET-backed web goal), and "
    "`apply_offline_conversions` is the bid-eligible phone-call path -- import calls by MSCLKID "
    "against an OfflineConversion goal (no native 'calls from ads' goal exists); "
