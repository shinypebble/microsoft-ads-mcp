...
The Microsoft Advertising API uses OAuth: an Azure AD app (client id) plus a developer token,
exchanged for a long-lived refresh token. We persist the refresh token so the server can run
non-interactively across restarts, and we mint one interactively via the auth tools when none
is configured.
"""
...
def make_grant(
    settings: Settings,
) -> OAuthDesktopMobileAuthCodeGrant | OAuthWebAuthCodeGrant:
    """Construct the right OAuth grant for the app registration type.

    A client secret implies a web (confidential) app; otherwise a desktop/native app.
    """
